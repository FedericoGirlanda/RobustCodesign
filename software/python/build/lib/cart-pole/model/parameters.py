...
class Cartpole:
    ...
    def continuous_dynamics2(self, x, u):
        D = 4 * self.M * self.r_mp ** 2 + self.Mp * self.r_mp ** 2 + 4 * self.Jm * self.Kg ** 2 \
            + 3 * self.Mp * self.r_mp ** 2 * np.sin(x[0, 1]) ** 2

        c_acc = - 3 * self.r_mp ** 2 * self.Bp * np.cos(x[0, 1]) * x[0, 3] / (self.lp * D) \
                - 4 * self.Mp * self.lp * self.r_mp ** 2 * np.sin(x[0, 1]) * x[0, 3] ** 2 / D \
                - 4 * self.r_mp ** 2 * self.Beq * x[0, 2] / D \
                + 3 * self.Mp * self.r_mp ** 2 * self.g * np.cos(x[0, 1]) * np.sin(x[0, 1]) / D \
                + 4 * self.r_mp**2 * u[0] / (D * self.eta_g * self.eta_m)

        p_acc = - 3 * (self.M * self.r_mp**2 + self.Mp * self.r_mp**2 + self.Jm * self.Kg**2) * self.Bp * x[0, 3] / (self.Mp * self.lp**2 * D) \
                - 3 * self.Mp * self.r_mp**2 * np.cos(x[0, 1]) * np.sin(x[0, 1]) * x[0, 3]**2 / D \
                - 3 * self.r_mp**2 * self.Beq * np.cos(x[0, 1]) * x[0, 2] / (self.lp * D) \
                + 3 * (self.M * self.r_mp**2 + self.Mp * self.r_mp**2 + self.Jm * self.Kg**2) * self.g * np.sin(x[0, 1]) / (self.lp * D) \
                + 3 * self.r_mp**2 * np.cos(x[0, 1]) * u[0] / (self.lp * D * self.eta_g * self.eta_m)

        xd = np.array([x[0, 2], x[0, 3], c_acc[0], p_acc[0]])
        return xd

    def continuous_dynamics3(self, x, u):
        D = 4 * self.M * self.r_mp ** 2 + self.Mp * self.r_mp ** 2 + 4 * self.Jm * self.Kg ** 2 \
