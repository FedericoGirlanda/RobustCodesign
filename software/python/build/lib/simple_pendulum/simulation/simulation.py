...
from pydrake.all import ( DiagramBuilder, LogVectorOutput, Saturation, AddMultibodyPlantSceneGraph, 
                            ExtractSimulatorConfig, ApplySimulatorConfig)
from pydrake.all import Simulator as drakeSim
...
class DrakeSimulator():
    def __init__(self, tv_controller,pendulum_plant, dt_sim, dt_log, mpar):
        ...
        self.torque_limit = mpar["tl"]
        self.dt_log = dt_log

        # simulation time step
        self.dt_sim = dt_sim

        # Setup pendulum plant
        builder = DiagramBuilder()
        pendulum_plant = pendulum_plant.pendulum_plant
        builder.AddSystem(pendulum_plant)
        

        # Setup step controller with saturation block 
        self.nominal_time = tv_controller.time
        self.nominal_state = tv_controller.state
        self.nominal_input = tv_controller.input
        self.tf = self.nominal_time[-1]
        self.t0 = self.nominal_time[0]
        self.tv_controller = tv_controller.tvlqr_sys     
        controller_plant = builder.AddSystem(self.tv_controller)
        saturation = builder.AddSystem(Saturation(min_value=[-self.torque_limit], max_value=[self.torque_limit]))
        ...
        builder.Connect(saturation.get_output_port(),
                        pendulum_plant.get_input_port())
        builder.Connect(pendulum_plant.get_output_port(),
                        controller_plant.get_input_port())     
        
        # Setup a logger for the acrobot state
        self.state_logger = LogVectorOutput(pendulum_plant.get_output_port(), builder, self.dt_log)
        self.input_logger = LogVectorOutput(saturation.get_output_port(), builder, self.dt_log)

        # Build-up the diagram
        self.diagram = builder.Build()

    def simulate(self, x0 = np.array([0,0]), init_knot = 0, final_knot = -1):

        # Set up a simulator to run this diagram
        self.simulator = drakeSim(self.diagram)

        from pydrake.systems.analysis import GetIntegrationSchemes, ResetIntegratorFromFlags
        #print(GetIntegrationSchemes())
        ResetIntegratorFromFlags(self.simulator,'runge_kutta3',max_step_size=self.dt_sim)
        integrator = self.simulator.get_mutable_integrator()
        integrator.set_fixed_step_mode(self.dt_sim)
        ...
        if init_knot != 0 or final_knot != -1: 
            context.SetTime(self.nominal_time[init_knot])
            self.simulator.AdvanceTo(self.nominal_time[final_knot])
        else:
            context.SetTime(self.t0)
            self.simulator.AdvanceTo(self.tf)

        x_sim = np.vstack((np.array(self.nominal_state).T[0],np.array(self.state_logger.FindLog(context).data()).T)).T
        u_sim = np.append(np.array(self.nominal_input).T[0],self.input_logger.FindLog(context).data())
        t_sim = np.append(self.nominal_time[0],self.input_logger.FindLog(context).sample_times()+self.dt_sim)
